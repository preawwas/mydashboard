...
export default function ExpenseDashboard() {
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {/* Header */}
            <div>
                <h2 className="text-xl font-semibold text-[#FAFAFA] mb-2">Expense Analytics</h2>
                <p className="text-[#A1A1AA]">วิเคราะห์ค่าใช้จ่ายของคุณ</p>
            </div>

            {/* Top Categories */}
            <div className="space-y-4">
                <h3 className="text-lg font-medium text-[#FAFAFA]">Top Layers</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    {categories.slice(0, 5).map((cat, index) => (
                        <div
                            key={index}
                            onClick={() => setSelectedCategory({ id: cat.id, name: cat.name })}
                            className="bg-[#1C1B16] border border-[#2E2C24] p-4 rounded-xl flex flex-col items-center justify-center gap-3 hover:border-[#F5C542] transition-colors cursor-pointer group"
                        >
                            <div className="w-12 h-12 rounded-full bg-[#15140F] flex items-center justify-center border border-[#2E2C24] group-hover:border-[#F5C542]">
                                <div className="w-6 h-6 rounded-full" style={{ backgroundColor: cat.color }} />
                            </div>
                            <div className="text-center">
                                <p className="text-sm text-[#FAFAFA] font-medium">{cat.name}</p>
                                <p className="text-xs text-[#A1A1AA]">฿{cat.amount.toLocaleString()}</p>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <p className="text-2xl font-bold text-[#FAFAFA]">฿{totalStats.toLocaleString()}</p>
                    </CardHeader>
                    <CardContent>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <p className="text-2xl font-bold text-[#FAFAFA]">Top Spending Breakdown</p>
                    </CardHeader>
                    <CardContent>
                        <div className="h-[300px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart layout="vertical" data={categories.slice(0, 5)} margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
